...
PM.PolygoneContainerCheck = function(){
  	if(PM_MainContainer.style.display == "block"){
 	 	AM.Aktiv = true;
 	 	AM_StartenButton.style.backgroundColor = "green";
 	 	AM_StartenButton.innerHTML = '►';
	  	PM.ClosePolygonMaker();
  	}else{
	  	AM.Aktiv = false;
	  	AM_StartenButton.style.backgroundColor = "#997600";
	  	AM_StartenButton.innerHTML = 'Stop';
 	 	PM_MainContainerAnzeigen();
  	}
}
PM_MainContainerAnzeigen = function(){
	PM_MainContainer.style.display = "block";
	PM.DatabaseObjects = [];
	PM.Polygons = [];
	PM.CreateDatabaseObjectsFromDataArray(faktuellerCharakter().Polygone.WelcheGibtEs);
...
PM.ClosePolygonMaker=function(){
	faktuellerCharakter().Polygone.WelcheGibtEs = PM.GetPolygonDataArray();
